...
export function QuarantinePage({tank}:{tank:Tank}) {
 ...
 function add(){
  ...
  const recognizedFish:typeof subjectCategory=!subjectCategory&&/(?:clown\\s*fish|سمك(?:ة)?\\s*المهرج)/i.test(organism)?"fish":subjectCategory;
  const recognizedCoral:typeof subjectCategory=!recognizedFish&&/(?:torch\\s*coral|مرجان|تورش)/i.test(organism)?"coral":recognizedFish;
