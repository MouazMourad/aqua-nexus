...
export function EquipmentPage({tank}:{tank:Tank}) {
 ...
 const remove=(id:string)=>patch(tank.id,t=>{const nextEquipment=t.equipment.filter(x=>x.id!==id);const synced=syncEquipmentSystem({...t,equipment:nextEquipment});return {...t,...synced};});
  ...
  <Modal open={!!e} title={e?.name??tr(lang,"equipment")} onClose={()=>setDetails(null)}>
   {e&&profile&&<>
    <div className="form-grid">
      ...
      <label className="field"><span>{bi(lang,"متوسط التشغيل ساعة/يوم","Average hours/day")}</span><input type="number" min="0" max="24" step=".1" value={e.hoursPerDay??""} onChange={ev=>updateDevice(e.id,x=>({...x,hoursPerDay:Math.min(24,Number(ev.target.value))}))}/></label><label className="field"><span>{bi(lang,"التدفق L/h","Flow L/h")}</span><input type="number" min="0" value={e.flowLph??""} onChange={ev=>updateDevice(e.id,x=>({...x,flowLph:Number(ev.target.value)||undefined}))}/></label><label className="field"><span>{bi(lang,"الحجم المصنف L","Rated volume L")}</span><input type="number" min="0" value={e.ratedVolumeLiters??""} onChange={ev=>updateDevice(e.id,x=>({...x,ratedVolumeLiters:Number(ev.target.value)||undefined}))}/></label>{e.kind==="lighting"&&<><label className="field"><span>PAR {bi(lang,"عند عمق الكائنات","at livestock depth")}</span><input type="number" min="0" value={e.parAtTargetDepth??""} onChange={ev=>updateDevice(e.id,x=>({...x,parAtTargetDepth:Number(ev.target.value)||undefined}))}/></label><label className="field"><span>{bi(lang,"تغطية الطول cm","Coverage length cm")}</span><input type="number" min="0" value={e.coverageLengthCm??""} onChange={ev=>updateDevice(e.id,x=>({...x,coverageLengthCm:Number(ev.target.value)||undefined}))}/></label><label className="field"><span>{bi(lang,"تغطية العرض cm","Coverage width cm")}</span><input type="number" min="0" value={e.coverageWidthCm??""} onChange={ev=>updateDevice(e.id,x=>({...x,coverageWidthCm:Number(ev.target.value)||undefined}))}/></label></>}<label className="field"><span>{bi(lang,"العمر المتوقع الأدنى / سنة","Expected life min / years")}</span><input type="number" min="0" step=".5" value={((e.expectedLifeMinDays??0)/365)||""} onChange={ev=>updateDevice(e.id,x=>({...x,expectedLifeMinDays:Math.round(Number(ev.target.value)*365)||undefined}))}/></label><label className="field"><span>{bi(lang,"العمر المتوقع الأعلى / سنة","Expected life max / years")}</span><input type="number" min="0" step=".5" value={((e.expectedLifeMaxDays??0)/365)||""} onChange={ev=>updateDevice(e.id,x=>({...x,expectedLifeMaxDays:Math.round(Number(ev.target.value)*365)||undefined}))}/></label><label className="field"><span>{bi(lang,"الأهمية","Criticality")}</span><select value={e.criticality??"normal"} onChange={ev=>updateDevice(e.id,x=>({...x,criticality:ev.target.value}))}><option value="critical">{bi(lang,"حرج","Critical")}</option><option value="important">{bi(lang,"مهم","Important")}</option><option value="normal">{bi(lang,"عادي","Normal")}</option></select></label><label className="field"><span>{bi(lang,"مجموعة الازدواجية","Redundancy group")}</span><input value={e.redundancyGroup??""} onChange={ev=>updateDevice(e.id,x=>({...x,redundancyGroup:ev.target.value}))}/></label><label className="field"><span>{bi(lang,"خطة Backup","Backup plan")}</span><input value={e.backupPlan??""} onChange={ev=>updateDevice(e.id,x=>({...x,backupPlan:ev.target.value}))}/></label><label className="field"><span>{bi(lang,"قطعة/جهاز احتياطي متوفر","Spare available")}</span><select value={e.spareAvailable?"yes":"no"} onChange={ev=>updateDevice(e.id,x=>({...x,spareAvailable:ev.target.value==="yes"}))}><option value="no">{bi(lang,"لا","No")}</option><option value="yes">{bi(lang,"نعم","Yes")}</option></select></label>
    </div>
