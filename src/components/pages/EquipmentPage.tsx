...
export function EquipmentPage({tank}:{tank:Tank}) {
 ...
 const [open,setOpen]=useState(false),[details,setDetails]=useState<string|null>(null),[name,setName]=useState(""),[kind,setKind]=useState<EquipmentKind>("lighting"),[location,setLocation]=useState<string>("display"),[brand,setBrand]=useState(""),[model,setModel]=useState(""),[days,setDays]=useState(90),[power,setPower]=useState(0),[hours,setHours]=useState(0);
 ...
 return <section className="page-grid">
  ...
  <Modal open={open} title={tr(lang,"addEquipment")} onClose={()=>setOpen(false)}>
   <div className="form-grid">
    ...
    <label className="field"><span>{bi(lang,"ساعات التشغيل / يوم","Hours / day")}</span><input type="number" min="0" max="24" step=".1" value={hours||""} onChange={e=>setHours(Number(e.target.value))}/></label>
   </div>
   <div className="inline-alert info">{bi(lang,"الموقع المقترح يتغير تلقائياً حسب نوع الجهاز ويمكنك تعديله قبل الحفظ. قيم الطاقة اختيارية ويمكن إضافتها لاحقاً.","A suggested location is selected by device type and can be changed before saving. Energy values are optional and can be added later.")}</div>
  ...
  <Modal open={!!e} title={e?.name??tr(lang,"equipment")} onClose={()=>setDetails(null)}>
   {e&&profile&&<>
    <div className="form-grid">
      ...
      <label className="field"><span>{bi(lang,"متوسط التشغيل ساعة/يوم","Average hours/day")}</span><input type="number" min="0" max="24" step=".1" value={e.hoursPerDay??""} onChange={ev=>updateDevice(e.id,x=>({...x,hoursPerDay:Math.min(24,Number(ev.target.value))}))}/></label>
    </div>
