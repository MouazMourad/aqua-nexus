  ...
  <Modal open={!!e} title={e?.name??tr(lang,"equipment")} onClose={()=>setDetails(null)}>
   {e&&profile&&<>
    <div className="form-grid">
      ...
      <label className="field"><span>{bi(lang,"متوسط التشغيل ساعة/يوم","Average hours/day")}</span><input type="number" min="0" max="24" step=".1" value={e.hoursPerDay??""} onChange={ev=>updateDevice(e.id,x=>({...x,hoursPerDay:Math.min(24,Number(ev.target.value))}))}/></label><label className="field"><span>{bi(lang,"التدفق L/h","Flow L/h")}</span><input type="number" min="0" value={e.flowLph??""} onChange={ev=>updateDevice(e.id,x=>({...x,flowLph:Number(ev.target.value)||undefined}))}/></label><label className="field"><span>{bi(lang,"الحجم المصنف L","Rated volume L")}</span><input type="number" min="0" value={e.ratedVolumeLiters??""} onChange={ev=>updateDevice(e.id,x=>({...x,ratedVolumeLiters:Number(ev.target.value)||undefined}))}/></label>{e.kind==="lighting"&&<><label className="field"><span>PAR {bi(lang,"عند عمق الكائنات","at livestock depth")}</span><input type="number" min="0" value={e.parAtTargetDepth??""} onChange={ev=>updateDevice(e.id,x=>({...x,parAtTargetDepth:Number(ev.target.value)||undefined}))}/></label><label className="field"><span>{bi(lang,"تغطية الطول cm","Coverage length cm")}</span><input type="number" min="0" value={e.coverageLengthCm??""} onChange={ev=>updateDevice(e.id,x=>({...x,coverageLengthCm:Number(ev.target.value)||undefined}))}/></label><label className="field"><span>{bi(lang,"تغطية العرض cm","Coverage width cm")}</span><input type="number" min="0" value={e.coverageWidthCm??""} onChange={ev=>updateDevice(e.id,x=>({...x,coverageWidthCm:Number(ev.target.value)||undefined}))}/></label></>}
    </div>
