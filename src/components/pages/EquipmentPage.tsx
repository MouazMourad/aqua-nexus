...
export function EquipmentPage({tank}:{tank:Tank}) {
 ...
 const [open,setOpen]=useState(false),[importOpen,setImportOpen]=useState(false),[details,setDetails]=useState<string|null>(null),[name,setName]=useState(""),[kind,setKind]=useState<EquipmentKind>("lighting"),[location,setLocation]=useState<string>("display"),[brand,setBrand]=useState(""),[model,setModel]=useState(""),[days,setDays]=useState(90),[power,setPower]=useState(0),[hours,setHours]=useState(0),[ratedVolume,setRatedVolume]=useState(0),[flowLph,setFlowLph]=useState(0),[par,setPar]=useState(0),[coverageLength,setCoverageLength]=useState(0),[coverageWidth,setCoverageWidth]=useState(0),[failureNote,setFailureNote]=useState(""),[advancedPosition,setAdvancedPosition]=useState(false),[positionPreview,setPositionPreview]=useState<{id:string;position:DisplayEquipmentPosition}|null>(null);
 ...
 function setEnergySettings(p:Partial<{pricePerKwh:number;currency:string}>){patch(tank.id,t=>{
  const next={pricePerKwh:t.energySettings?.pricePerKwh??0,currency:t.energySettings?.currency||"USD",...p};
  const check=validateEnergySettings(next);
 ...
 return <section className="page-grid">
  ...
  <div className="full-span"><AdvancedSection titleAr="العمر والاحتياط وقطع الاستهلاك" titleEn="Lifecycle, redundancy & consumables" summaryAr="تفاصيل الاعتمادية وقطع الاحتياط؛ افتحها عند مراجعة المخاطر أو التخطيط للاستبدال." summaryEn="Reliability and spare-part detail; open it for risk review or replacement planning." defaultOpen={false}><div style={{display:"grid",gap:10,paddingTop:10}}><ContextHint id="equipment-reliability" lang={lang} ar="الجهاز الشغال اليوم ممكن يظل نقطة فشل واحدة. هالقسم يضيف العمر والأعطال والبديل للمشهد، بدون ما يزاحم الحالة اليومية." en="A device working today can still be a single point of failure. This section adds lifecycle, failures and backup context without crowding daily status."/><section className="card panel full-span">
  ...
  <div className="full-span"><AdvancedSection titleAr="الطاقة والتكلفة" titleEn="Energy & cost" summaryAr="تحليل اختياري للواط وساعات التشغيل والتكلفة الشهرية." summaryEn="Optional analysis of watts, operating hours and monthly cost." defaultOpen={false}><div style={{paddingTop:10}}><section className="card panel full-span">
  ...
  <Modal open={!!e} title={e?.name??tr(lang,"equipment")} onClose={()=>setDetails(null)}>
   {e&&profile&&<>
    <div className="form-grid">
      ...
      <label className="field"><span>{tr(lang,"serviceInterval")}</span><input type="number" value={e.serviceIntervalDays??90} onChange={ev=>updateDevice(e.id,x=>({...x,serviceIntervalDays:sanitizeBounded(Number(ev.target.value),1,3650,x.serviceIntervalDays??90)}))}/></label>
      <label className="field"><span>{bi(lang,"القدرة W","Power W")}</span><input type="number" min="0" value={e.powerWatts??""} onChange={ev=>{const v=Number(ev.target.value);if(v<0){window.alert(bi(lang,"القدرة لا يمكن أن تكون سالبة.","Power cannot be negative."));return;}updateDevice(e.id,x=>({...x,powerWatts:sanitizeNonNegative(v,x.powerWatts??0,100000)}))}}/></label>
      <label className="field"><span>{bi(lang,"متوسط التشغيل ساعة/يوم","Average hours/day")}</span><input type="number" min="0" max="24" step=".1" value={e.hoursPerDay??""} onChange={ev=>updateDevice(e.id,x=>({...x,hoursPerDay:sanitizeBounded(Number(ev.target.value),0,24,x.hoursPerDay??0)}))}/></label><label className="field"><span>{bi(lang,"التدفق L/h","Flow L/h")}</span><input type="number" min="0" value={e.flowLph??""} onChange={ev=>updateDevice(e.id,x=>({...x,flowLph:sanitizeNonNegative(Number(ev.target.value),x.flowLph??0,20000000)||undefined}))}/></label><label className="field"><span>{bi(lang,"الحجم المصنف L","Rated volume L")}</span><input type="number" min="0" value={e.ratedVolumeLiters??""} onChange={ev=>updateDevice(e.id,x=>({...x,ratedVolumeLiters:sanitizeNonNegative(Number(ev.target.value),x.ratedVolumeLiters??0,5000000)||undefined}))}/></label>{e.kind==="co2"&&<><label className="field"><span>{bi(lang,"نمط CO₂","CO₂ mode")}</span><select value={e.co2Mode??"injected"} onChange={ev=>updateDevice(e.id,x=>({...x,co2Mode:ev.target.value as any}))}><option value="injected">{bi(lang,"حقن CO₂","Injected CO₂")}</option><option value="lowTech">Low-tech</option></select></label><label className="field"><span>{bi(lang,"المتبقي بالأسطوانة %","Cylinder remaining %")}</span><input type="number" min="0" max="100" value={e.co2CylinderRemainingPercent??""} onChange={ev=>updateDevice(e.id,x=>({...x,co2CylinderRemainingPercent:sanitizeBounded(Number(ev.target.value),0,100,x.co2CylinderRemainingPercent??0)}))}/></label><label className="field"><span>Drop checker</span><select value={e.co2DropChecker??"unknown"} onChange={ev=>updateDevice(e.id,x=>({...x,co2DropChecker:ev.target.value as any}))}><option value="unknown">—</option><option value="blue">Blue</option><option value="green">Green</option><option value="yellow">Yellow</option></select></label></>}{e.kind==="lighting"&&<><label className="field"><span>PAR {bi(lang,"عند عمق الكائنات","at livestock depth")}</span><input type="number" min="0" value={e.parAtTargetDepth??""} onChange={ev=>updateDevice(e.id,x=>({...x,parAtTargetDepth:sanitizeNonNegative(Number(ev.target.value),x.parAtTargetDepth??0,5000)||undefined}))}/></label><label className="field"><span>{bi(lang,"تغطية الطول cm","Coverage length cm")}</span><input type="number" min="0" value={e.coverageLengthCm??""} onChange={ev=>updateDevice(e.id,x=>({...x,coverageLengthCm:sanitizeNonNegative(Number(ev.target.value),x.coverageLengthCm??0,5000)||undefined}))}/></label><label className="field"><span>{bi(lang,"تغطية العرض cm","Coverage width cm")}</span><input type="number" min="0" value={e.coverageWidthCm??""} onChange={ev=>updateDevice(e.id,x=>({...x,coverageWidthCm:sanitizeNonNegative(Number(ev.target.value),x.coverageWidthCm??0,5000)||undefined}))}/></label></>}<label className="field"><span>{bi(lang,"العمر المتوقع الأدنى / سنة","Expected life min / years")}</span><input type="number" min="0" step=".5" value={((e.expectedLifeMinDays??0)/365)||""} onChange={ev=>updateDevice(e.id,x=>({...x,expectedLifeMinDays:Math.round(Number(ev.target.value)*365)||undefined}))}/></label><label className="field"><span>{bi(lang,"العمر المتوقع الأعلى / سنة","Expected life max / years")}</span><input type="number" min="0" step=".5" value={((e.expectedLifeMaxDays??0)/365)||""} onChange={ev=>updateDevice(e.id,x=>({...x,expectedLifeMaxDays:Math.round(Number(ev.target.value)*365)||undefined}))}/></label><label className="field"><span>{bi(lang,"الأهمية","Criticality")}</span><select value={e.criticality??"normal"} onChange={ev=>updateDevice(e.id,x=>({...x,criticality:ev.target.value}))}><option value="critical">{bi(lang,"حرج","Critical")}</option><option value="important">{bi(lang,"مهم","Important")}</option><option value="normal">{bi(lang,"عادي","Normal")}</option></select></label><label className="field"><span>{bi(lang,"مجموعة الازدواجية","Redundancy group")}</span><input value={e.redundancyGroup??""} onChange={ev=>updateDevice(e.id,x=>({...x,redundancyGroup:ev.target.value}))}/></label><label className="field"><span>{bi(lang,"خطة Backup","Backup plan")}</span><input value={e.backupPlan??""} onChange={ev=>updateDevice(e.id,x=>({...x,backupPlan:ev.target.value}))}/></label><label className="field"><span>{bi(lang,"قطعة/جهاز احتياطي متوفر","Spare available")}</span><select value={e.spareAvailable?"yes":"no"} onChange={ev=>updateDevice(e.id,x=>({...x,spareAvailable:ev.target.value==="yes"}))}><option value="no">{bi(lang,"لا","No")}</option><option value="yes">{bi(lang,"نعم","Yes")}</option></select></label>
    </div>
    ...
    <section className="card panel" style={{marginTop:14}}>
      ...
      {(e.consumables??[]).length?(e.consumables??[]).map(c=><div className="history-row" key={c.id}><div><b>{lang==="ar"?c.name:(c.nameEn||c.name)}</b><small>{bi(lang,"عمر مرجعي","Reference life")}: {c.lifeDays??"—"} {bi(lang,"يوم","days")}</small></div><label className="field" style={{maxWidth:120}}><span>{bi(lang,"المخزون","Stock")} ({c.unit??"pc"})</span><input type="number" min="0" value={c.quantityOnHand??0} onChange={ev=>updateDevice(e.id,x=>({...x,consumables:(x.consumables??[]).map((z:any)=>z.id===c.id?{...z,quantityOnHand:sanitizeNonNegative(Number(ev.target.value),z.quantityOnHand??0,1000000000)}:z)}))}/></label><button className="btn" onClick={()=>updateDevice(e.id,x=>({...x,consumables:(x.consumables??[]).map((z:any)=>z.id===c.id?{...z,installedAt:today()}:z)}))}>{bi(lang,"تم الاستبدال","Replaced")}</button></div>):<div className="inline-alert info">{bi(lang,"ما في قطع استهلاك افتراضية لهالنوع.","No default consumables for this equipment type.")}</div>}
    </section>
    <section className="card panel" style={{marginTop:14}}>
