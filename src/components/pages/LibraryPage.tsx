...
export function LibraryPage({tank}:{tank:Tank}) {
 ...
 const add=(x:any)=>{
  ...
  patch(tank.id,t=>({...t,livestock:[...t.livestock,{id:uid("live"),libraryId:x.id,name:x.ar,nameEn:x.en,category:String(x.cat).toLowerCase()==="fish"?"fish":String(x.cat).toLowerCase()==="coral"?"coral":String(x.cat).toLowerCase()==="invert"?"invert":String(x.cat).toLowerCase()==="plant"?"plant":"other",quantity:1,health:"good",load:x.load,addedAt:nowISO()}],timeline:[{id:uid("ev"),timestamp:nowISO(),type:"livestock",textAr:`تمت إضافة 1 × ${x.ar} من المكتبة بعد فحص الجاهزية المركزي.`,textEn:`Added 1 × ${x.en} from the library after central readiness screening.`},...t.timeline]}));
 };
 return <section className="page-grid"><PageHeader eyebrow="LIVESTOCK LIBRARY" title={tr(lang,"library")}/>
 <div className="full-span"><ContextHint id="library-readiness" lang={lang} ar="حكم الجاهزية هون خاص بحوضك الحالي: الحجم والكائنات والبيانات المسجلة. مو حكم عام إن النوع مناسب أو غير مناسب لكل الأحواض." en="Readiness here is specific to your current tank—volume, livestock and recorded evidence—not a universal judgment about the species."/></div>
