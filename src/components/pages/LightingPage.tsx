...
export function LightingPage({tank,onEquipment}:{tank:Tank;onEquipment:()=>void}){
 ...
 return <section className="page-grid lighting-page">
  <PageHeader eyebrow="LIGHTING INTELLIGENCE" title={bi(lang,"الإنارة الذكية","Lighting Intelligence")} actions={<button className="btn primary" onClick={saveProgram} disabled={!dirty}>{dirty?bi(lang,"حفظ البرنامج","Save program"):bi(lang,"البرنامج محفوظ","Program saved")}</button>}/>

  <section className="card panel full-span lighting-command">
   <div className="module-head"><div><small className="eyebrow-mini">TANK BRAIN • LIGHT</small><h3>{bi(lang,"الضوء كجزء من حالة الحوض، مو مجرد مؤقت إنارة","Light as part of tank state, not just a timer")}</h3><p className="note">{bi(lang,"Aqua Nexus يقرأ البرنامج، مواقع وحدات الإنارة، أبعاد الحوض، معايرة PAR والكيمياء ليعطيك صورة واحدة قابلة للفهم.","Aqua Nexus combines the schedule, fixture positions, tank geometry, PAR calibration and chemistry into one readable model.")}</p></div><span className={"status "+levelClass(intel.level)}>{intel.level.toUpperCase()}</span></div>
  ...
  <section className="card panel full-span">
   ...
   <div className="lighting-controls">
    <label className="field"><span>{bi(lang,"وقت المحاكاة","Simulation time")} • {formatLightMinute(viewMinute)}</span><input type="range" min="0" max="1439" step="5" value={viewMinute} onChange={e=>setViewMinute(Number(e.target.value))}/></label>
    <label className="field"><span>{bi(lang,"عمق خريطة Top View","Top-map depth")} • {Math.round(depthPct)}%</span><input type="range" min="0" max="100" step="5" value={depthPct} onChange={e=>setDepthPct(Number(e.target.value))}/></label>
    <div className="inline-alert info"><b>{bi(lang,"الشدة المركبة الآن","Composite intensity now")}: {Math.round(at.weightedPercent)}%</b><p>{bi(lang,"النموذج يحسب مساهمة القنوات حسب الوزن الضوئي، وليس جمع النسب بشكل أعمى.","The model weights channel contribution instead of blindly summing percentages.")}</p></div>
   </div>
  ...
  <section className="card panel full-span">
   ...
   <div className="lighting-brain-grid">
    ...
    <div><h4>{bi(lang,"التبخر وتعويض الماء","Evaporation & top-off")}</h4><div className="inline-alert info">{bi(lang,"الإنارة الأطول/الأقوى قد ترفع الحرارة والتبخر، لكن Aqua Nexus لن يخمّن كمية التعويض بدون سجل حجم ATO/تعويض ماء فعلي. هاد النوع من البيانات رح يدخل لاحقاً ضمن طبقة الاستيراد الموحدة.","Longer/stronger lighting can increase heat and evaporation, but Aqua Nexus will not invent top-off volume without measured ATO/top-off history. That data can later enter through the unified import layer.")}</div></div>
