     ...
     {(["top","mid","bottom"] as const).map(zone=>{
      ...
      return <div className={"lighting-depth-zone depth-"+zone} key={zone}>
       ...
       <div className="lighting-depth-residents">
        {residents.length?residents.map(x=><article className={"lighting-depth-resident "+(x.placementStatus==="outside"?"outside":x.placementStatus==="within"?"within":"suggested")} data-livestock-id={x.livestockId} data-placement-status={x.placementStatus} data-light-par={typeof x.estimatedPeakParAtActualDepth==="number"?x.estimatedPeakParAtActualDepth.toFixed(2):""} key={x.livestockId}>
