...
import { tr,bi,eventTypeText } from "@/i18n";
...
export function TimelinePage({tank}:{tank:Tank}) {
 ...
 <div className="filter-bar full-span"><label className="field"><span>{bi(lang,"نوع الحدث","Event type")}</span><select value={type} onChange={e=>setType(e.target.value)}><option value="all">{tr(lang,"all")}</option>{types.map(x=>
  <option key={x} value={x}>
    {eventTypeText(lang,x)}
  </option>
)}</select></label><label className="field grow"><span>{tr(lang,"search")}</span><input value={search} onChange={e=>setSearch(e.target.value)}/></label></div>
 ...
 <div className="timeline full-span">{rows.length?rows.map(x=><article className="timeline-item" key={x.id}><span className="timeline-dot"/><div><small>{new Date(x.timestamp).toLocaleString()}</small><b>{lang==="ar"?x.textAr:x.textEn}</b><em>
  {eventTypeText(lang,x.type)}
  {x.source==="intelligence"?" • Tank Brain":""}
</em></div></article>):<div className="empty-state">{search.trim()?bi(lang,"لا توجد نتائج مطابقة للبحث.","No results match your search."):tr(lang,"timelineEmpty")}</div>}{nextCursor&&<button className="btn" onClick={loadMore}>{bi(lang,"عرض 200 حدث أقدم","Show 200 older events")}</button>}</div>
