...
export function TimelinePage({tank}:{tank:Tank}) {
 ...
 const types=useMemo(()=>[...new Set(tank.timeline.map(x=>x.type))].sort(),[tank.timeline]);
 const filtered=useMemo(()=>tank.timeline.filter(x=>(type==="all"||x.type===type)&&(!search||`${x.textAr} ${x.textEn} ${x.type}`.toLowerCase().includes(search.toLowerCase()))),[tank.timeline,type,search]);
