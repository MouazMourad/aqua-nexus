...
export function BreedingPage({tank}:{tank:Tank}){
 ...
 const markStable=(id:string)=>{const ts=nowISO();patch(tank.id,t=>({...t,livestock:t.livestock.map(x=>x.id===id?{...x,health:"good",lastObservedAt:ts,lifeEvents:[{id:uid("life"),timestamp:ts,type:"health",note:lang==="ar"?"اكتمل النمو الأولي وأصبح الناتج مستقراً.":"Initial establishment completed; propagation result is stable."},...(x.lifeEvents??[])]}:x),timeline:[{id:uid("ev"),timestamp:ts,type:"propagation-stable",textAr:"التكاثر • تم تأكيد استقرار الناتج وانتقاله من المتابعة الأولية إلى كائن مستقر.",textEn:"Propagation • Result confirmed stable and moved from establishment monitoring to stable livestock."},...t.timeline]}))};
