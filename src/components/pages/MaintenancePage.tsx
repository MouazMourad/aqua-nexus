...
export function MaintenancePage({tank}:{tank:Tank}) {
 ...
 const done=(id:string)=>{
  ...
  patch(tank.id,t=>({...t,maintenance:t.maintenance.map(x=>x.id===id?{...x,done:true,lastDone:today(),nextDue:new Date(Date.now()+(x.intervalDays??days[x.cadence])*86400000).toISOString().slice(0,10),checklistDone:x.checklist?.map((_,i)=>i)??x.checklistDone}:x)}));
 };
 ...
 return <section className="page-grid maintenance-page">
  ...
  <div className="card panel full-span maintenance-plan-card">
   {cadences.map(c=><div className="maintenance-group" key={c}><h3>{tr(lang,c)}</h3><div className="task-list">{tasks.filter(x=>x.cadence===c).map(x=><div className="task-row printable-task" key={x.id}><span className={`check-dot ${x.done?"done":""}`}>{x.done?"✓":""}</span><div><b>{(lang==="ar"?x.title:(x.titleEn||x.title)).replace("[TRAVEL] ","")}</b><small>{tr(lang,"due")}: {x.nextDue??"—"} {x.lastDone?` • ${tr(lang,"lastDone")}: ${x.lastDone}`:""} {x.autoGenerated?` • AUTO • ${x.sourceEquipmentName||""}`:""}</small></div><span className={`task-print-status ${x.done?"done":"pending"}`}>{x.done?tr(lang,"completed"):tr(lang,"pendingTasks")}</span><button className="btn no-print" onClick={()=>setTaskDetails(x.id)}>{bi(lang,"قائمة","Checklist")}</button><button className="btn good no-print" onClick={()=>done(x.id)}>✓</button></div>)}</div></div>)}
