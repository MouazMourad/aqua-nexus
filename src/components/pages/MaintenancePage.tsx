...
export function MaintenancePage({tank}:{tank:Tank}) {
 ...
 return <section className="page-grid maintenance-page">
  ...
  <div className="card panel full-span maintenance-plan-card">
   {cadences.map(c=><div className="maintenance-group" key={c}><h3>{tr(lang,c)}</h3><div className="task-list maintenance-task-list">{tasks.filter(x=>x.cadence===c).map(x=>{const effective=maintenanceEffectiveState(x);return <div className={`task-row printable-task maintenance-task-row ${effective.completed?"is-complete":""}`} key={x.id}><span className={`maintenance-state-dot ${effective.completed?"done":""}`} aria-hidden="true">{effective.completed?"✓":""}</span><div className="maintenance-task-copy"><b>{(lang==="ar"?x.title:(x.titleEn||x.title)).replace("[TRAVEL] ","")}</b><small>{tr(lang,"due")}: {x.nextDue??"—"} {x.lastDone?` • ${tr(lang,"lastDone")}: ${x.lastDone}`:""} {x.autoGenerated?` • AUTO • ${x.sourceEquipmentName||""}`:""}</small></div><div className="maintenance-task-actions"><span className={`task-print-status ${effective.completed?"done":"pending"}`}>{effective.completed?tr(lang,"completed"):tr(lang,"pendingTasks")}</span>{(x.checklist??[]).length>0&&<button className="maintenance-checklist-btn no-print" onClick={()=>setTaskDetails(x.id)}>☷ <span>{bi(lang,"قائمة","Checklist")}</span></button>}<button className="maintenance-complete-btn no-print" onClick={()=>done(x.id)} aria-label={bi(lang,"تعليم المهمة كمكتملة","Mark task complete")}><span>✓</span><em>{bi(lang,"تم","Done")}</em></button></div></div>})}</div></div>)}
