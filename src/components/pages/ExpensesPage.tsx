...
export function ExpensesPage({tank}:{tank:Tank}) {
 ...
 <div className="card panel"><ContextHint id="expenses-currency" lang={lang} ar="لحتى الإجماليات تكون صحيحة، سجّل المصاريف بنفس العملة أو غيّر العملة فقط لما تكون القيم السابقة محسوبة بنفسها." en="For meaningful totals, keep expenses in one currency or change currency only when prior entries use the same basis."/><div className="form-grid"><label className="field"><span>{tr(lang,"description")}</span><input value={desc} onChange={e=>setDesc(e.target.value)}/></label><label className="field"><span>{tr(lang,"amount")}</span><input type="number" min="0" max="1000000000" step="any" value={amount} onChange={e=>setAmount(Number(e.target.value))}/></label><label className="field"><span>{tr(lang,"currency")}</span><input value={currency} onChange={e=>setCurrency(e.target.value)}/></label></div><button className="btn primary" onClick={add} disabled={!desc.trim()||amount<=0}>{tr(lang,"addExpense")}</button><div className="summary-strip" style={{marginTop:12}}><div className="summary"><small>{tr(lang,"totalExpenses")}</small><b>{total.toFixed(2)} {currency}</b></div><div className="summary"><small>{lang==="ar"?"هذا الشهر":"This month"}</small><b>{monthly.toFixed(2)} {currency}</b></div><div className="summary"><small>{lang==="ar"?"طاقة شهرية تقديرية":"Estimated monthly energy"}</small><b>{energy.monthlyKwh.toFixed(1)} kWh</b></div></div></div>
