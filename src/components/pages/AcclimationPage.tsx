 ...
 return <section className="page-grid acclimation-page">
  ...
  {step<5&&<div className="card acclimation-wizard full-span">
    ...
    <div className="acclimation-wizard-body">
      ...
      <div className="acclimation-default-summary"><div><small>{bi(lang,"الإعداد المطبق حالياً","CURRENT PROFILE")}</small><b>{categoryText(lang,category)} • {drip} {bi(lang,"دقيقة إقلمة","min acclimation")} • {interval} {bi(lang,"دقيقة فاصل","min release interval")}</b><span>{bi(lang,`${temperamentLabel(lang,temperament)} • ${sensitivityLabel(lang,sensitivity)}`,`${temperamentLabel(lang,temperament)} • ${sensitivityLabel(lang,sensitivity)}`)}</span></div><button className="btn" onClick={()=>setShowItemAdvanced(v=>!v)}>{showItemAdvanced?bi(lang,"إخفاء التفاصيل","Hide details"):bi(lang,"تعديل التفاصيل / Advanced","Adjust details / Advanced")}</button></div>
