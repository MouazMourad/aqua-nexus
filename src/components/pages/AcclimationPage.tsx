...
export function AcclimationPage({tank}:{tank:Tank}) {
 ...
 return <section className="page-grid acclimation-page">
  ...
  {step>=5&&<>
   ...
   <section className="full-span"><div className="section-title"><div><h2>{bi(lang,"طابور التنزيل / التثبيت","Release / placement queue")}</h2><p className="note">{bi(lang,"الترتيب والدفعات مقترحة حسب الحساسية، حالة الوصول والسلوك. انتهاء العداد يعني جاهز للفحص وليس يجب التنزيل.","Order and batches are suggested from sensitivity, arrival condition and temperament. Timer ending means ready for inspection, not mandatory release.")}</p></div></div>{batchSummary.length>1&&<div className="release-batch-plan"><div className="release-batch-plan-head"><b>{bi(lang,"خطة التنزيل على دفعات","Suggested release batches")}</b><small>{releasePlan.length} {bi(lang,"مجموعة","groups")} • {batchSummary.length} {bi(lang,"دفعات","batches")}</small></div><div className="release-batch-chips">{batchSummary.map(x=><span key={x.batch} className={x.sensitive?"has-sensitive":""}><b>{bi(lang,`دفعة ${x.batch}`,`Batch ${x.batch}`)}</b><small>{x.count} {bi(lang,"مجموعة","groups")}{x.sensitive?` • ${x.sensitive} ${bi(lang,"حساسة","sensitive")}`:""}</small></span>)}</div><p>{bi(lang,"الدفعات تنظيمية وليست إجبارية؛ الكائن المجهد أو الحرج يبقى بحاجة فحص وقرار فردي قبل التنزيل.","Batches are organizational, not mandatory; stressed or critical livestock still needs individual assessment before release.")}</p></div>}<div className="acclimation-queue">{releasePlan.map(({item,order,batch,totalBatches})=>{const rem=remaining(item,now);return <article className={`acclimation-item ${item.status}`} key={item.id}><div className="batch-ribbon"><b>{bi(lang,`دفعة ${batch}`,`Batch ${batch}`)}</b>{totalBatches>1&&<small>{batch}/{totalBatches}</small>}</div>{item.imageDataUrl?<img className="acclimation-item-photo" src={item.imageDataUrl} alt=""/>:<div className="acclimation-item-photo placeholder">{icon[item.category]}</div>}<div className="itembody"><div className="itemhead"><div><small>{categoryText(lang,item.category)} • {bi(lang,`ترتيب #${order}`,`Order #${order}`)}</small><h3>{lang==="ar"?item.name:(item.nameEn||item.name)} ×{item.quantity}</h3></div><span className={`status ${item.status}`}>{acclimationStatusLabel(lang,item.status)}</span></div><div className="meta"><span className="pill">{healthLabel(lang,item.health)}</span>{item.subtype&&<span className="pill">{subtypeLabel(lang,item.subtype)}</span>}<span className="pill">{temperamentLabel(lang,item.temperament||"peaceful")}</span><span className="pill">{sensitivityLabel(lang,item.sensitivity||"normal")}</span></div><p className="placement">📍 {item.placement||"—"}</p>{["acclimating","paused","ready"].includes(item.status)&&<div className="item-timer">{fmt(rem)}</div>}<div className="acclimation-actions">{item.status==="waiting"&&<button className="btn primary" disabled={!active.floatConfirmed} onClick={()=>startDrip(item.id)}>{bi(lang,"تم النقل — ابدأ التنقيط","Transferred — start drip")}</button>}{item.status==="acclimating"&&<button className="btn" onClick={()=>itemAction(item.id,"pause")}>{bi(lang,"إيقاف","Pause")}</button>}{item.status==="paused"&&<button className="btn primary" onClick={()=>itemAction(item.id,"resume")}>{bi(lang,"استئناف","Resume")}</button>}{["acclimating","paused","ready"].includes(item.status)&&<><button className="btn" onClick={()=>itemAction(item.id,"plus5")}>+5</button><button className="btn" onClick={()=>itemAction(item.id,"plus15")}>+15</button></>}{["acclimating","paused"].includes(item.status)&&<button className="btn good" onClick={()=>itemAction(item.id,"ready")}>{bi(lang,"جاهز للفحص","Ready for check")}</button>}{item.status==="ready"&&<button className="btn primary" onClick={()=>markAdded(item)}>{bi(lang,"نقل الكائن فقط — بدون ماء الشحنة","Transfer animal only — no shipping water")}</button>}{!['added','deferred'].includes(item.status)&&<button className="btn warn" onClick={()=>itemAction(item.id,"defer")}>{bi(lang,"تخطي / تأجيل","Skip / defer")}</button>}{!['added','deferred'].includes(item.status)&&<button className="btn danger" onClick={()=>startEmergency(item.id)}>🚨 {bi(lang,"إضافة استثنائية","Add exception")}</button>}</div></div></article>})}</div></section>
