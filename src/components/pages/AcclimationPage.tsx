...
export function AcclimationPage({tank}:{tank:Tank}) {
 ...
 useEffect(()=>{
  ...
  const onVisibility=()=>{if(document.visibilityState==="visible"){setNow(Date.now());void acquire();}};
  void acquire();
 ...
 function exportTxt(){const blob=new Blob([registryText()],{type:"text/plain;charset=utf-8"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`Aqua_Nexus_Acclimation_${today()}.txt`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
  ...
  {step>=5&&<>
    ...
    <div className="batch-board-grid">
     {releaseBatches.map(batch=>{const runtime=batchRuntime(batch);const expanded=Boolean(expandedBatches[batch.id]);const visibleItems=runtime.remainingItems.filter((i:AcclimationItem)=>!i.emergency);const isReady=runtime.status==="ready";const dipRun=batch.lane==="coral"&&active.coralDipEnabled?coralDipBatchRun(active,batch.id):undefined;return <article className={`batch-box ${runtime.allAdded?"success":runtime.status}`} key={batch.id}>
      ...
      {expanded&&!runtime.allAdded&&<div className="batch-box-body">
       ...
       {visibleItems.length===0?<div className="batch-empty">{bi(lang,"لا توجد عناصر متبقية في هذه الدفعة ضمن المسار العام.","No remaining items in this normal batch.")}</div>:visibleItems.map((item:AcclimationItem)=>{const transferGate=coralTransferGate(active,item);const timerFinished=(item.status==="acclimating"||item.status==="emergency")&&Boolean(item.endAt)&&Number(item.endAt)<=now;const itemReady=item.status==="ready"||timerFinished;return <div className={`batch-livestock-row ${itemReady?"ready":item.status}`} key={item.id}><span className="batch-live-icon">{icon[item.category]}</span><div><b>{lang==="ar"?item.name:(item.nameEn||item.name)} ×{item.quantity}</b><small>{healthLabel(lang,item.health)} • {sensitivityLabel(lang,item.sensitivity||"normal")}{item.subtype?` • ${subtypeLabel(lang,item.subtype)}`:""}</small>{item.category==="coral"&&active.coralDipEnabled&&!transferGate.allowed&&<small>🪸 {lang==="ar"?transferGate.reasonAr:transferGate.reasonEn}</small>}</div><div className="batch-live-actions">{!itemReady?<span className="waiting-chip">⏱ {bi(lang,"بانتظار انتهاء عداد هذه الدفعة فقط","Waiting for this batch timer only")}</span>:transferGate.allowed?<button className="btn primary" onClick={()=>markAdded(item)}>✓ {bi(lang,"تم النقل إلى الحوض","Transferred to tank")}</button>:<span className="waiting-chip">🪸 {bi(lang,"بانتظار Dip / Rinse","Waiting for Dip / Rinse")}</span>}{!["added","deferred"].includes(item.status)&&<button className="btn danger" onClick={()=>startEmergency(item.id)}>🚨 {bi(lang,"استثناء","Exception")}</button>}</div></div>})}
      </div>}
     </article>})}
    </div>
   </section>}
   <details className="card panel full-span acclimation-session-details" open={Boolean(sessionCompleted)}>
    ...
    <div className="acclimation-session-detail-body">
     ...
     <section className="acclimation-event-log"><div className="section-title"><h2>{tr(lang,"eventLog")}</h2></div><div className="history-list">{active.events.map(x=><div className="history-row" key={x.id}><b>{lang==="ar"?x.textAr:x.textEn}</b><span>{new Date(x.timestamp).toLocaleString()}</span></div>)}</div></section>
    </div>
   </details>
  </>}
  <style jsx>{`
