...
export function SumpPage({tank}:{tank:Tank}) {
 ...
 const config=(field:"length"|"width"|"height",value:number)=>patch(tank.id,t=>{
  const safe=Math.max(1,value),old=Number(t.sump.dimensions[field])||safe,ratio=safe/old;
  const chambers=dimensionPolicy==="scale"?t.sump.chambers.map(ch=>{
  ...
  return {...t,sump:{...t.sump,dimensions:{...t.sump.dimensions,[field]:safe},chambers}};
 });
 ...
 {tank.sump.enabled&&<div className="chamber-grid sump-chamber-grid full-span">{tank.sump.chambers.map((x,i)=>{
  ...
  const contentParts=[...(x.media??[]).map(m=>mediaLabel(lang,m)),...(x.items??[]),x.contents].filter(Boolean);
 ...
 <section className="card panel full-span sump-media-section">
