...
import { inventoryCategoryLabel,inventoryProfile,unifiedInventory } from "@/domain/inventoryIntelligence";
...
export function InventoryPage({tank}:{tank:Tank}) {
 ...
 const [category,setCategory]=useState<InventoryCategory>("other"),[subcategory,setSubcategory]=useState(""),[compatibility,setCompatibility]=useState<"marine"|"freshwater"|"both">(tank.type),[dosingParameter,setDosingParameter]=useState<"KH"|"Ca"|"Mg">("KH"),[dosingCompoundId,setDosingCompoundId]=useState("");
 ...
 const reset=()=>{setPreset("");setCustom("");setQty(0);setUnit("g");setMin(0);setCategory("other");setSubcategory("");setCompatibility(tank.type);setDosingParameter("KH");setDosingCompoundId("")};
 ...
 const add=()=>{
  ...
  patch(tank.id,t=>({...t,inventory:[...t.inventory,{
   ...
   inventorySubcategory:preset?inferred.subcategory:(subcategory.trim()||"other"),
