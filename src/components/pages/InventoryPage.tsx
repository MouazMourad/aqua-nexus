...
export function InventoryPage({tank}:{tank:Tank}) {
 ...
 <Modal open={open} title={tr(lang,"addStock")} onClose={()=>{setOpen(false);reset()}}>
  ...
  <AdvancedSection titleAr="تصنيف وربط متقدم" titleEn="Advanced classification & linking" summaryAr="تفاصيل تساعد الوحدات الثانية تعرف كيف تستخدم المادة وتنبهك قبل نفادها." summaryEn="Details that help other modules understand how to use this item and alert you before it runs low.">
   <div style={{display:"grid",gap:9,paddingTop:10}}>
    <ContextHint id="inventory-classification" lang={lang} ar="التصنيف الدقيق بيخلي الجرعات والإقلمة والماء والمعدات تشوف المادة المناسبة فقط بدل عرض كل المخزون." en="Precise classification lets dosing, acclimation, water-change and equipment modules see only compatible stock."/>
    <div className="form-grid">
     {!preset&&<><label className="field"><span>{bi(lang,"النوع الفرعي","Subcategory")}</span><select value={subcategory} onChange={e=>{setSubcategory(e.target.value);if(e.target.value!=="other")setCustomSubcategory("")}}>{inventorySubcategoryOptions(category).map(x=><option key={x.value} value={x.value}>{lang==="ar"?x.ar:x.en}</option>)}</select></label>{subcategory==="other"&&<label className="field"><span>{bi(lang,"نوع مخصص (اختياري)","Custom subtype (optional)")}</span><input value={customSubcategory} onChange={e=>setCustomSubcategory(e.target.value)}/></label>}<label className="field"><span>{bi(lang,"متوافق مع","Tank compatibility")}</span><select value={compatibility} onChange={e=>setCompatibility(e.target.value as "marine"|"freshwater"|"both")}><option value="marine">{bi(lang,"بحري","Marine")}</option><option value="freshwater">{bi(lang,"نهري","Freshwater")}</option><option value="both">{bi(lang,"الاثنين","Both")}</option></select></label>{category==="dosing"&&<><label className="field"><span>{bi(lang,"عنصر التصحيح","Correction parameter")}</span><select value={dosingParameter} onChange={e=>setDosingParameter(e.target.value as "KH"|"Ca"|"Mg")}><option>KH</option><option>Ca</option><option>Mg</option></select></label><label className="field"><span>{bi(lang,"المادة الدقيقة للحاسبة (اختياري)","Exact calculator compound (optional)")}</span><select value={dosingCompoundId} onChange={e=>setDosingCompoundId(e.target.value)}><option value="">{bi(lang,"منتج تجاري / غير محدد","Commercial / unspecified")}</option>{DOSING_PRESETS.filter(x=>x.parameter===dosingParameter).map(x=><option key={x.id} value={x.id}>{lang==="ar"?x.ar:x.en} — {x.formula}</option>)}</select></label></>}</>}
