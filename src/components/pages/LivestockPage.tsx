...
export function LivestockPage({tank,onLibrary}:{tank:Tank;onLibrary:()=>void}) {
 ...
 const confirmRemove=()=>{if(!removeId)return;patch(tank.id,t=>{const item=t.livestock.find(x=>x.id===removeId),ts=nowISO();if(!item)return t;const exit={id:uid("exit"),timestamp:ts,livestockId:item.id,name:item.name,nameEn:item.nameEn,category:item.category,quantity:item.quantity,reason:exitReason,bodyRemoved:exitReason==="death"?bodyRemoved:undefined,symptoms:exitSymptoms||undefined,notes:exitNotes||undefined};const reasonAr=exitReason==="death"?"وفاة":exitReason==="sold"?"بيع":exitReason==="transferred"?"نقل":exitReason==="returned"?"إرجاع":"إزالة";const reasonEn=exitReason==="death"?"death":exitReason==="sold"?"sold":exitReason==="transferred"?"transferred":exitReason==="returned"?"returned":"removed";const chambers=t.sump.chambers.map(ch=>ch.refugium?{...ch,refugium:{...ch.refugium,livestockIds:ch.refugium.livestockIds.filter(id=>id!==removeId)}}:ch);const livestock=t.livestock.filter(x=>x.id!==removeId).map(x=>x.parentLivestockId===removeId?{...x,parentLivestockId:undefined}:x);return {...t,livestock,sump:{...t.sump,chambers},livestockExits:[exit,...(t.livestockExits??[])],timeline:[{id:uid("ev"),timestamp:ts,type:`livestock-${exitReason}`,textAr:`${reasonAr}: ${item.quantity} × ${item.name}.`,textEn:`${item.quantity} × ${item.nameEn||item.name} ${reasonEn}.`},...t.timeline]};});setRemoveId(null)};
