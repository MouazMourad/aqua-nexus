...
export function TrainingCoach({tank,onNavigate}:{tank:Tank;onNavigate:(page:AppPage)=>void}){
 ...
 return <section className="training-coach">
  ...
  <div className="training-challenge-card">
   <div className="training-challenge-copy"><span>🎯</span><div><small>AQUA NEXUS CHALLENGE</small><b>{lang==="ar"?"اختبر قراراتك وتنافس بالنقاط":"Test your decisions and compete on score"}</b><p>{lang==="ar"?"6 مراحل: مبتدئ → متوسط → متقدم. كل قرار بيأثر على صحة الحوض، وبالنهاية فيك تشارك نتيجتك.":"6 stages: Beginner → Intermediate → Advanced. Every decision affects tank health, and you can share your final score."}</p>
    <div className="training-only-note">
     <strong>{lang==="ar"?"🎓 متاح في وضع التدريب فقط":"🎓 Available in Training Mode only"}</strong>
     <span>{lang==="ar"?"هذا التحدي يعمل داخل الحوض الافتراضي فقط، ومخصص للتعلّم والتجربة ومشاركة النتائج. لا يظهر في الأحواض الحقيقية ولا يؤثر على بياناتك الفعلية.":"This challenge is available only inside the virtual training tank. It is designed for learning, experimentation, and sharing results. It does not appear in real aquariums and does not affect your real data."}</span>
    </div>
   </div></div>
   <button className="btn primary" onClick={()=>setChallengeOpen(true)}>🏆 {lang==="ar"?"ابدأ التحدي":"Start Challenge"}</button>
  </div>
  ...
  <TrainingChallenge tank={tank} open={challengeOpen} onClose={()=>setChallengeOpen(false)}/>
  ...
  <style jsx>{`
   ...
   .training-challenge-card{margin-bottom:10px;padding:12px;border:1px solid rgba(255,200,90,.18);border-radius:16px;background:radial-gradient(circle at 90% 0%,rgba(255,200,90,.10),transparent 42%),rgba(255,255,255,.022);display:flex;align-items:center;justify-content:space-between;gap:14px}.training-challenge-copy{display:flex;gap:10px;align-items:flex-start}.training-challenge-copy>span{width:36px;height:36px;display:grid;place-items:center;border-radius:11px;background:rgba(255,200,90,.09);font-size:18px}.training-challenge-copy small,.training-challenge-copy b{display:block}.training-challenge-copy small{font-size:8px;letter-spacing:.13em;color:#ffc85a;font-weight:900}.training-challenge-copy b{font-size:13px;margin:2px 0}.training-challenge-copy p{margin:0;font-size:10px;line-height:1.5;opacity:.58}
   .training-only-note{margin-top:10px;padding:10px 12px;border-radius:14px;border:1px solid rgba(255,200,90,.22);background:rgba(255,200,90,.08);display:grid;gap:4px}
   .training-only-note strong{font-size:12px;color:#ffd36a;font-weight:900}
   .training-only-note span{font-size:11px;line-height:1.65;opacity:.82}
   ...
   @media(max-width:720px){.training-coach{margin:10px 10px 2px;padding:12px}.training-challenge-card{align-items:stretch;flex-direction:column}.training-challenge-card>.btn{width:100%}.training-coach-head{grid-template-columns:1fr}.training-score{display:grid;grid-template-columns:auto auto 1fr;align-items:center;gap:8px;text-align:start}.training-score b{font-size:18px;margin:0}.training-score span{text-align:end}.training-missions{grid-template-columns:1fr}.training-coach-foot{align-items:flex-start;flex-direction:column}.training-coach-foot .btn{width:100%}}
  `}</style>
 </section>;
}
