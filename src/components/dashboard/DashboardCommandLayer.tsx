...
export function DashboardCommandLayer(){
 ...
 return createPortal(<>
  ...
  <section className="aqua-command-center card panel">
   ...
   <div className="today-actions">{data.today.length?data.today.slice(0,3).map((x,i)=><button key={i} onClick={()=>navigate(x.page)}><span>{x.icon}</span><b>{language==="ar"?x.ar:x.en}</b><i>›</i></button>):data.risks.length?<div className="today-clear">✓ <b>{language==="ar"?"الإجراءات المطلوبة ظاهرة في التنبيه أعلاه":"Required actions are shown in the alert above"}</b></div>:<div className="today-clear">✓ <b>{language==="ar"?"ما في إجراء ضروري اليوم":"No action is required today"}</b></div>}</div>
