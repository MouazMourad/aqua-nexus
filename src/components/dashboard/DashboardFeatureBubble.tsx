...
export function DashboardFeatureBubble({onNavigate}:{onNavigate:(page:AppPage)=>void}){
  ...
  useEffect(()=>{
    ...
    setMuted(sessionStorage.getItem("aqua-feature-bubbles-muted")==="1");
  },[]);
  ...
  const label=useMemo(()=>{
    ...
    return names[tip.page]?.[lang==="ar"?"ar":"en"]??tip.page;
  },[tip.page,lang]);
  ...
  function mute(){
    setMuted(true);
    if(typeof window!=="undefined")sessionStorage.setItem("aqua-feature-bubbles-muted","1");
  }

  return <div className="feature-bubble-lane" aria-live="polite">
    <div className="feature-bubble" key={cycle}>
      <button type="button" className="feature-bubble-close" onClick={mute} aria-label={lang==="ar"?"إخفاء تلميحات هذه الجلسة":"Hide tips for this session"}>×</button>
      <button type="button" className="feature-bubble-body" onClick={()=>onNavigate(tip.page)}>
    ...
    <style jsx>{`
      ...
      .feature-bubble{position:absolute;inset-inline-end:clamp(12px,4vw,58px);bottom:-190px;width:min(360px,calc(100vw - 28px));pointer-events:auto;animation:aquaBubbleRise 5s linear forwards;filter:drop-shadow(0 14px 28px rgba(0,0,0,.28))}
      ...
      .feature-bubble-body{width:100%;border:1px solid rgba(134,228,248,.28);border-radius:32px 32px 32px 18px;background:linear-gradient(135deg,rgba(17,75,98,.66),rgba(5,31,46,.54));backdrop-filter:blur(13px);-webkit-backdrop-filter:blur(13px);color:inherit;padding:13px 34px 13px 14px;display:grid;grid-template-columns:38px 1fr;gap:9px;text-align:inherit;cursor:pointer;box-shadow:inset 0 1px 0 rgba(255,255,255,.09)}
      ...
      .feature-bubble-close{position:absolute;z-index:2;inset-inline-end:9px;top:9px;width:21px;height:21px;border-radius:50%;border:1px solid rgba(255,255,255,.11);background:rgba(1,18,27,.42);color:#c7eaf0;display:grid;place-items:center;cursor:pointer}
      @keyframes aquaBubbleRise{
        0%{transform:translate3d(0,0,0) scale(.88);opacity:0}
        7%{opacity:.94}
        42%{transform:translate3d(-10px,-38vh,0) scale(1)}
        82%{opacity:.9}
        94%{transform:translate3d(8px,-82vh,0) scale(1.04);opacity:.8}
        100%{transform:translate3d(8px,-90vh,0) scale(1.22);opacity:0}
      }
