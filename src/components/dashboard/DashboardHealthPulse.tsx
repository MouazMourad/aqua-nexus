...
export function DashboardHealthPulse(){
  ...
  const intelligence=useMemo(()=>{
    ...
    if(state.band==="critical"){
      if(criticalEquipment)actions.push({
        ar:`تدخل فوري: افحص ${criticalEquipment.name} واستمرارية دوران الماء/الحرارة قبل أي خطوة ثانية.`,
        en:`Immediate action: check ${criticalEquipment.name} and water circulation/temperature before anything else.`,
        focus:"equipment",urgent:true
      });
      else if(!latest||chemistryIssues.length)actions.push({
        ar:"تدخل فوري: أعد قياس القيم غير الطبيعية الآن وتأكد من النتيجة قبل أي تصحيح كبير أو جرعة إضافية.",
        en:"Immediate action: retest abnormal values now and confirm them before any large correction or extra dosing.",
        focus:"chemistry",urgent:true
      });
    }

    if(!latest)actions.push({
      ar:"سجّل فحص كيمياء كامل الآن؛ ما في قراءة حديثة يمكن الاعتماد عليها.",
      en:"Log a complete chemistry test now; there is no current reading to rely on.",
      focus:"chemistry",urgent:state.band==="critical"
    });
    else if(age>7)actions.push({
      ar:`أعد فحص الكيمياء اليوم؛ آخر قراءة عمرها ${Math.floor(age)} يوم.`,
      en:`Retest chemistry today; the latest reading is ${Math.floor(age)} days old.`,
      focus:"chemistry",urgent:state.band==="critical"
    });
    ...
    if(chemistryIssues.length){
      ...
      actions.push({
        ar:`راجع ${issue.key} أولاً: القراءة ${compactNumber(issue.value)} والمجال المثالي ${compactNumber(issue.ideal[0])}–${compactNumber(issue.ideal[1])}. عدّل عامل واحد فقط وراقب الاستجابة.`,
        en:`Review ${issue.key} first: current ${compactNumber(issue.value)}, ideal ${compactNumber(issue.ideal[0])}–${compactNumber(issue.ideal[1])}. Change one factor at a time and watch the response.`,
        focus:"chemistry",urgent:state.band==="critical"&&issue.score<=25
      });
    }

    if(overdue.length)actions.push({
      ar:`أنجز مهمة الصيانة الأقرب: ${overdue[0].title} ثم حدّث سجلها.`,
      en:`Complete the nearest maintenance task: ${overdue[0].titleEn||overdue[0].title}, then update its log.`,
      focus:"maintenance"
    });

    if(equipmentWarnings.length&&!criticalEquipment)actions.push({
      ar:`افحص ${equipmentWarnings[0].name} لأنه مسجّل كجهاز يحتاج انتباهاً/صيانة.`,
      en:`Check ${equipmentWarnings[0].name}; it is marked for attention/service.`,
      focus:"equipment"
    });

    if(bio.status==="high"||bio.status==="danger")actions.push({
      ar:"أوقف إضافة كائنات جديدة مؤقتاً وراجع الحمل الحيوي وكفاءة الترشيح.",
      en:"Pause new livestock additions and review bioload and filtration capacity.",
      focus:"bioload"
    });

    if(livestockWarnings.length)actions.push({
      ar:`راقب ${livestockWarnings[0].name} بشكل قريب وسجّل أي تغير قبل تعديل أكثر من عامل بالحوض.`,
      en:`Watch ${livestockWarnings[0].nameEn||livestockWarnings[0].name} closely and log any change before adjusting multiple tank factors.`,
      focus:"bioload"
    });

    if(activeAcclimation)actions.push({
      ar:"هناك أقلمة نشطة؛ تجنّب تغييرات كبيرة بالحوض إلى أن تكتمل وتستقر الكائنات.",
      en:"An acclimation session is active; avoid major tank changes until it is complete and livestock settles."
    });

    if(!actions.length){
      actions.push({
        ar:state.band==="excellent"?"ما في إجراء تصحيحي الآن؛ حافظ على نفس الروتين ولا تغيّر شيئاً لمجرد رفع الرقم.":"الحالة مستقرة؛ حافظ على الروتين الحالي وراقب القراءة القادمة بدل إجراء تعديل غير ضروري.",
        en:state.band==="excellent"?"No corrective action is needed now; keep the routine and do not change things just to raise the score.":"The tank is stable; keep the current routine and watch the next reading instead of making an unnecessary adjustment."
      });
    }
  ...
  },[tank?.id,health,chemistry,maintenance,state?.score]);

  if(!tank||!state||!intelligence)return null;

  const stateLabel:Record<HealthTone,{ar:string;en:string}>={
    excellent:{ar:"ممتاز",en:"Excellent"},
    stable:{ar:"مستقر",en:"Stable"},
    watch:{ar:"يحتاج متابعة",en:"Needs attention"},
    stressed:{ar:"متوتر",en:"Stressed"},
  ...
  const lastUpdate=(()=>{
    ...
    const relative=diffMinutes<1?(language==="ar"?"الآن":"now")
      :diffMinutes<60?(language==="ar"?`منذ ${diffMinutes} دقيقة`:`${diffMinutes}m ago`)
      :diffMinutes<1440?(language==="ar"?`منذ ${Math.floor(diffMinutes/60)} ساعة`:`${Math.floor(diffMinutes/60)}h ago`)
      :(language==="ar"?`منذ ${Math.floor(diffMinutes/1440)} يوم`:`${Math.floor(diffMinutes/1440)}d ago`);
  ...
  const heroPortal=targets.hero?createPortal(
    <section className="aqua-now-panel" aria-label={language==="ar"?"حالة الحوض وما يجب فعله الآن":"Tank status and what to do now"}>
      <div className="aqua-now-head">
        <div className="aqua-now-title">
          <span className="aqua-now-status-dot" aria-hidden="true"/>
          <div>
            <small>{language==="ar"?"حالة اليوم":"TODAY"}</small>
            <b>{language==="ar"?stateLabel[currentTone].ar:stateLabel[currentTone].en} • {state.score}%</b>
          </div>
        </div>
        <span className={`aqua-now-signal ${intelligence.attentionCount?"has-alert":"clear"}`}>
          {intelligence.attentionCount?`⚠ ${intelligence.attentionCount} ${language==="ar"?"نقطة متابعة":"to review"}`:`✓ ${language==="ar"?"لا تنبيه فعّال":"No active alert"}`}
        </span>
      </div>

      <div className="aqua-daily-status">
        <b>{language==="ar"?"ملخص اليوم":"Daily status"}</b>
        <span>{language==="ar"?state.ar:state.en} • {trendLabel}</span>
      </div>

      {intelligence.primaryReason&&<div className="aqua-health-reason">
        <small>{language==="ar"?"السبب الأهم":"Main driver"}</small>
        <span>{language==="ar"?intelligence.primaryReason.ar:intelligence.primaryReason.en}</span>
      </div>}

      <div className="aqua-now-actions-wrap">
        <div className="aqua-now-section-title">
          <b>{language==="ar"?"شو أعمل الآن؟":"What should I do now?"}</b>
          {currentTone==="critical"&&<span className="aqua-urgent-label">{language==="ar"?"تدخل فوري":"Immediate"}</span>}
        </div>
        <div className="aqua-now-actions">
          {intelligence.actions.map((action,index)=><button key={`${action.ar}-${index}`} type="button" className={`aqua-action ${action.urgent?"urgent":""} ${action.focus?"clickable":""}`} onClick={()=>focusModule(action.focus)} disabled={!action.focus}>
            <span>{action.urgent?"!":index+1}</span>
            <b>{language==="ar"?action.ar:action.en}</b>
          </button>)}
        </div>
      </div>

      <div className="aqua-now-meta">
        <span>◷ {language==="ar"?"آخر تحديث":"Last updated"}: <b>{lastUpdate}</b></span>
        <button type="button" className="aqua-breakdown-toggle" aria-expanded={breakdownOpen} onClick={()=>setBreakdownOpen(value=>!value)}>
          {language==="ar"?"تفاصيل الصحة":"Health breakdown"} {breakdownOpen?"−":"+"}
        </button>
      </div>

      {breakdownOpen&&<div className="aqua-health-breakdown">
        <div><span><small>{language==="ar"?"الكيمياء":"Chemistry"} • 70%</small><b>{chemistry}%</b></span><i><u style={{width:`${chemistry}%`}}/></i></div>
        <div><span><small>{language==="ar"?"الصيانة":"Maintenance"} • 30%</small><b>{maintenance}%</b></span><i><u style={{width:`${maintenance}%`}}/></i></div>
        <div><span><small>{language==="ar"?"حالة النظام مع السياق":"System state with context"}</small><b>{state.score}%</b></span><i><u style={{width:`${state.score}%`}}/></i></div>
        <p>{language==="ar"?"Tank Health محسوبة من 70% كيمياء + 30% صيانة. حالة النظام الأوسع تأخذ أيضاً التجهيزات، الحمل الحيوي، والكائنات تحت المراقبة بالحسبان.":"Tank Health is 70% chemistry + 30% maintenance. The broader system state also considers equipment, bioload, and livestock under watch."}</p>
      </div>}
    </section>,
    targets.hero
  ):null;

  const chemistryPortal=targets.chemistryCopy?createPortal(
    <span className="aqua-chem-trends" aria-label={language==="ar"?"اتجاه آخر قياسين":"Trend across the latest two readings"}>
      {intelligence.chemistryTrends.length?intelligence.chemistryTrends.map(item=>{
        const direction=Math.abs(item.delta)<0.0001?"flat":item.delta>0?"up":"down";
        const quality=item.scoreDelta>4?"better":item.scoreDelta<-4?"worse":"flat";
        return <span key={item.key} className={`aqua-chem-trend ${quality}`} title={`${item.key}: ${compactNumber(item.current)} (${item.delta>=0?"+":""}${compactNumber(item.delta)})`}>
          <small>{item.key}</small><b>{direction==="up"?"↗":direction==="down"?"↘":"→"} {item.delta===0?"0":`${item.delta>0?"+":""}${compactNumber(item.delta)}`}</b>
        </span>;
      }):<span className="aqua-chem-trend flat"><small>{language==="ar"?"الترند":"Trend"}</small><b>{language==="ar"?"بعد القراءة الثانية":"after 2nd reading"}</b></span>}
    </span>,
    targets.chemistryCopy
  ):null;
  ...
  return <>
    ...
    <style jsx global>{`
      ...
      .pd-hero[data-aqua-health-tone]{
        display:flex;
        flex-direction:column;
        border-color:rgba(var(--health-rgb),.28)!important;
        background:
          radial-gradient(circle at 88% 0%,rgba(var(--health-rgb),.16),transparent 34%),
      ...
      .pd-hero[data-aqua-health-tone] .pd-hero-main{order:1}
      .pd-hero[data-aqua-health-tone] .aqua-now-panel{order:2}
      .pd-hero[data-aqua-health-tone] .pd-hero-footer{order:3;justify-content:flex-end!important}
      .pd-hero[data-aqua-health-tone] .pd-hero-footer>span{display:none!important}
      ...
      .aqua-now-panel{
        margin-top:14px;
        padding:13px 14px;
        border:1px solid rgba(var(--health-rgb),.24);
        border-radius:16px;
        background:linear-gradient(145deg,rgba(var(--health-rgb),.075),rgba(255,255,255,.025));
        box-shadow:inset 0 1px 0 rgba(255,255,255,.035);
        display:grid;
        gap:10px;
      }
      .aqua-now-head,.aqua-now-title,.aqua-now-meta,.aqua-now-section-title{display:flex;align-items:center;gap:9px}
      .aqua-now-head,.aqua-now-meta,.aqua-now-section-title{justify-content:space-between}
      .aqua-now-title>div{display:flex;flex-direction:column;gap:1px}.aqua-now-title small{font-size:9px;letter-spacing:.14em;opacity:.58}.aqua-now-title b{font-size:14px}
      .aqua-now-status-dot{width:10px;height:10px;border-radius:50%;background:var(--health-color);box-shadow:0 0 16px rgba(var(--health-rgb),.5);animation:healthHeartbeat var(--health-pulse-speed) ease-in-out infinite}
      .aqua-now-signal{font-size:11px;font-weight:850;padding:6px 8px;border-radius:999px;border:1px solid rgba(var(--health-rgb),.22);background:rgba(var(--health-rgb),.07);color:var(--health-color)}
      .aqua-daily-status{display:flex;align-items:center;gap:7px;flex-wrap:wrap;font-size:12px}.aqua-daily-status b{color:var(--health-color)}.aqua-daily-status span{opacity:.8}
      .aqua-health-reason{display:grid;grid-template-columns:auto 1fr;gap:9px;align-items:start;padding:9px 10px;border-radius:12px;background:rgba(0,0,0,.12);border:1px solid rgba(255,255,255,.05);font-size:12px}.aqua-health-reason small{font-weight:850;color:var(--health-color)}.aqua-health-reason span{opacity:.85;line-height:1.45}
      .aqua-now-actions-wrap{display:grid;gap:7px}.aqua-now-section-title b{font-size:13px}.aqua-urgent-label{font-size:10px;font-weight:900;color:#fff;background:rgba(var(--health-rgb),.72);padding:4px 7px;border-radius:999px;box-shadow:0 0 16px rgba(var(--health-rgb),.2)}
      .aqua-now-actions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px}.aqua-action{min-width:0;border:1px solid rgba(255,255,255,.07);border-radius:12px;background:rgba(255,255,255,.025);color:inherit;padding:8px 9px;display:grid;grid-template-columns:22px 1fr;gap:7px;align-items:start;text-align:start}.aqua-action>span{width:22px;height:22px;display:grid;place-items:center;border-radius:7px;background:rgba(var(--health-rgb),.11);color:var(--health-color);font-weight:900}.aqua-action b{font-size:11px;line-height:1.42;font-weight:730}.aqua-action.clickable{cursor:pointer}.aqua-action.clickable:hover{border-color:rgba(var(--health-rgb),.32);background:rgba(var(--health-rgb),.065)}.aqua-action.urgent{border-color:rgba(var(--health-rgb),.36);background:rgba(var(--health-rgb),.09)}.aqua-action:disabled{opacity:1;cursor:default}
      ...
      @media(max-width:720px){
        .aqua-now-actions{grid-template-columns:1fr}.aqua-health-breakdown{grid-template-columns:1fr}.aqua-health-breakdown p{grid-column:auto}.aqua-health-reason{grid-template-columns:1fr;gap:3px}
      }
      @media(max-width:620px){
        .aqua-now-panel{padding:11px;margin-top:11px}.aqua-now-head{align-items:flex-start}.aqua-now-signal{max-width:46%;text-align:center}.aqua-now-meta{align-items:flex-start}.aqua-chem-trends{gap:3px}.aqua-chem-trend{padding:2px 4px}
      }
      @media(prefers-reduced-motion:reduce){
        .pd-health-score[data-aqua-health-tone],
        .pd-module[data-aqua-health-tone],
        .aqua-now-status-dot{animation:none!important}
      }
    `}</style>
  </>;
}
