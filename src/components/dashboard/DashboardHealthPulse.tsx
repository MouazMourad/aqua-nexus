...
export function DashboardHealthPulse(){
  ...
  const heroPortal=targets.hero&&showDetails?createPortal(
    <section id="aqua-now-panel" className={`aqua-now-panel ${criticalForced?"is-forced-critical":""}`} aria-label={language==="ar"?"حالة الحوض وما يجب فعله الآن":"Tank status and what to do now"}>
      ...
      {breakdownOpen&&<div className="aqua-health-breakdown"><div><span><small>{language==="ar"?"الكيمياء":"Chemistry"} • 70%</small><b>{chemistry}%</b></span><i><u style={{width:`${chemistry}%`}}/></i></div><div><span><small>{language==="ar"?"الصيانة":"Maintenance"} • 30%</small><b>{maintenance}%</b></span><i><u style={{width:`${maintenance}%`}}/></i></div><div><span><small>{language==="ar"?"حالة النظام مع السياق":"System state with context"}</small><b>{state.score}%</b></span><i><u style={{width:`${state.score}%`}}/></i></div><p>{language==="ar"?"Tank Health محسوبة من 70% كيمياء + 30% صيانة. حالة النظام الأوسع تأخذ أيضاً التجهيزات، الحمل الحيوي، والكائنات تحت المراقبة بالحسبان.":"Tank Health is 70% chemistry + 30% maintenance. The broader system state also considers equipment, bioload, and livestock under watch."}</p></div>}
