...
export function AquaDashboardContent({tank,onNavigate}:{tank:Tank;onNavigate:(p:AppPage)=>void}) {
 ...
 return <div className="dashboard-final progressive-dashboard">
  <section className={`card panel pd-hero state-${state.band}`}>
   ...
   <div className="pd-intelligence-proof"><b>✦ {lang==="ar"?"تحليل مترابط للحوض":"Connected tank analysis"}</b><span>{knownComponents.length}/{system.components.length} {lang==="ar"?"محاور صحة لديها بيانات":"health domains with evidence"}</span><span>{lang==="ar"?"ثقة البيانات":"Data confidence"} {core.dataConfidence}%</span><span>{activeActionCount} {lang==="ar"?"إجراء ذكي نشط":"active smart action(s)"}</span></div>
