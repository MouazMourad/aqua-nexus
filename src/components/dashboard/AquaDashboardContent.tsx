...
export function AquaDashboardContent({tank,onNavigate}:{tank:Tank;onNavigate:(p:AppPage)=>void}) {
 ...
 const openActions=core.actions.slice(0,3);
 ...
 return <div className="dashboard-final progressive-dashboard">
  ...
  {openActions.length>0&&<section className="card panel pd-now-actions"><div className="module-head"><div><small className="eyebrow-mini">AQUA NEXUS • NEXT</small><h3>{lang==="ar"?"شو لازم أعمل هلق؟":"What should I do now?"}</h3></div><button className="btn" onClick={()=>onNavigate("alerts")}>{lang==="ar"?"كل التنبيهات":"All alerts"}</button></div><div className="pd-now-list">{openActions.map((a,i)=><button type="button" key={a.id} className={`pd-now-action ${a.level}`} onClick={()=>onNavigate(a.page as AppPage)}><span className="pd-now-rank">{i+1}</span><span><b>{lang==="ar"?a.ar:a.en}</b><small>{a.level==="danger"?(lang==="ar"?"أولوية عالية":"High priority"):a.level==="warn"?(lang==="ar"?"يحتاج انتباه":"Needs attention"):(lang==="ar"?"متابعة":"Follow-up")}</small></span><i>→</i></button>)}</div></section>}
