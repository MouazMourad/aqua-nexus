...
export function AquaDashboardContent({tank,onNavigate}:{tank:Tank;onNavigate:(p:AppPage)=>void}) {
 ...
 const [expanded,setExpanded]=useState<ModuleId|null>(null),[customizing,setCustomizing]=useState(false),[sceneView,setSceneView]=useState<"display"|"system">("display");
 ...
 function detailsFor(id:ModuleId){
  ...
  if(id==="digitalTwin")return <div className="pd-detail pd-scene-detail"><div className="pd-scene-modes"><button className={`btn ${sceneView==="display"?"primary":""}`} onClick={()=>setSceneView("display")}>{lang==="ar"?"الحوض":"Tank"}</button><button className={`btn ${sceneView==="system"?"primary":""}`} onClick={()=>setSceneView("system")}>{lang==="ar"?"النظام الكامل + اتجاه الماء":"Full system + flow"}</button></div><div className="card scene-card dashboard-scene-card"><SafeAquariumScene tank={tank} view={sceneView}/></div></div>;
 ...
 return <div className="dashboard-final progressive-dashboard">
  ...
  <div className="pd-module-grid">{visibleOrder.map(id=>{const open=expanded===id,s=summary[id],l=labels[id];return <section className={`card panel pd-module ${open?"is-open":""}`} key={id}><button type="button" className="pd-module-button" onClick={()=>setExpanded(open?null:id)}><span className="pd-module-icon">{l.icon}</span><span className="pd-module-copy"><small>{lang==="ar"?l.ar:l.en}</small><b>{s.value}</b><em>{s.note}</em></span><span className={`pd-module-state ${s.level??""}`}>{open?"−":"+"}</span></button>{open&&detailsFor(id)}</section>})}</div>
