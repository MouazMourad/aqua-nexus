...
export function AquaDashboardContent({tank,onNavigate}:{tank:Tank;onNavigate:(p:AppPage)=>void}) {
 ...
 const [expanded,setExpanded]=useState<ModuleId|null>(null),[customizing,setCustomizing]=useState(false),[sceneMode,setSceneMode]=useState<SceneMode>("tank"),[shareOpen,setShareOpen]=useState(false);
 ...
 return <div className="dashboard-final progressive-dashboard">
  ...
  <style jsx global>{`
   ...
   @media(max-width:620px){.progressive-dashboard{gap:10px}.pd-hero{padding:14px}.pd-hero-main{align-items:flex-start}.pd-mood-line p{font-size:12px}.pd-health-score{min-width:102px;padding:10px}.pd-health-score b{font-size:31px}.pd-hero-footer{align-items:stretch;flex-direction:column}.pd-hero-actions{width:100%}.pd-hero-actions .btn{flex:1}.pd-module-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.pd-module{border-radius:15px}.pd-module-button{min-height:104px;padding:11px;grid-template-columns:32px 1fr 23px;gap:7px}.pd-module-icon{width:32px;height:32px;border-radius:10px;font-size:16px}.pd-module-copy b{font-size:21px}.pd-module-copy em{font-size:11px}.pd-module.is-open{grid-column:1/-1}.pd-customizer{padding:11px}.progressive-dashboard .aquarium-scene-wrap{min-height:360px}}
  `}</style>
 </div>;
}
