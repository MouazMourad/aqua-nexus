...
export function AquaDashboardContent({tank,onNavigate}:{tank:Tank;onNavigate:(p:AppPage)=>void}) {
 ...
 const importedData=core.deviceData;
 ...
 function openSmartImport(){
  if(typeof window!=="undefined")sessionStorage.setItem("aqua-open-equipment-import","1");
 ...
 return <div className="dashboard-final progressive-dashboard">
  ...
  {shareOpen&&<section className="card panel pd-inline-share"><div className="module-head"><h3>↗ {lang==="ar"?"مشاركة حالة الحوض":"Share tank status"}</h3><button className="icon-btn" onClick={()=>setShareOpen(false)}>×</button></div><TankHealthShareCard tank={tank}/></section>}\n\n  <section className="card panel pd-smart-import">\n   <div className="pd-smart-import-main">\n    <div className="pd-smart-import-icon">⇧</div>\n    <div>\n     <small className="eyebrow-mini">AQUA NEXUS • SMART IMPORT</small>\n     <h3>{lang==="ar"?"استيراد ذكي لبيانات الحوض":"Smart aquarium data import"}</h3>\n     <p>{lang==="ar"?"ارفع Screenshot أو ملف Export من جهاز أو Controller. Aqua Nexus يحلله، يعطيك مسودة قابلة للتعديل، وبعد اعتمادك يوزّع كل معلومة تلقائياً على مكانها ويربطها بعقل الحوض.":"Upload a controller screenshot or export. Aqua Nexus analyzes it into an editable review draft, then routes approved data to the correct modules and Tank Brain."}</p>\n    </div>\n   </div>\n   <div className="pd-smart-import-flow" aria-label={lang==="ar"?"مسار الاستيراد الذكي":"Smart import routing"}>\n    <span>⚗ {lang==="ar"?"كيمياء":"Chemistry"}</span><span>⚙ {lang==="ar"?"تجهيزات":"Equipment"}</span><span>滴 {lang==="ar"?"جرعات":"Dosing"}</span><span>≈ ATO</span><span>⌁ Telemetry</span><span>△ {lang==="ar"?"تنبيهات":"Alerts"}</span><b>→ Tank Brain ✦</b>\n   </div>\n   <div className="pd-smart-import-footer">\n    <div>\n     <small>{lang==="ar"?"آخر استيراد":"Latest import"}</small>\n     <b>{latestImport?.vendor??(lang==="ar"?"لسه ما صار استيراد":"No import yet")}</b>\n     <span>{latestImport?(Math.round(latestImport.confidence)+"% "+(lang==="ar"?"ثقة":"confidence")+" • "+importedData.recentTelemetry+" Telemetry • "+importedData.unacknowledgedAlerts+" Alerts"):(lang==="ar"?"صورة أو CSV / JSON / TXT / XML / LOG":"Screenshot or CSV / JSON / TXT / XML / LOG")}</span>\n    </div>\n    <button type="button" className="btn primary" onClick={openSmartImport}>⇧ {lang==="ar"?"استيراد بيانات الحوض":"Import tank data"}</button>\n   </div>\n  </section>\n
