...
export function AquaAIAssistant({tank,page,onNavigate}:{tank:Tank;page:AppPage;onNavigate?:(page:AppPage)=>void}){
 ...
 function createPlan(){
  ...
  patch(tank.id,t=>({...t,aiActionPlans:[plan,...((t as any).aiActionPlans??[])],timeline:[{id:uid("ev"),timestamp:ts,type:"ai-action-plan",textAr:`Tank Intelligence أنشأ خطة متابعة: ${plan.titleAr}`,textEn:`Tank Intelligence created an action plan: ${plan.titleEn}`},...t.timeline]} as any));
 ...
 const greetingText=lang==="ar"?`Tank Intelligence عم يقرأ حالة ${tank.name} وتاريخه واتجاهاته بشكل مستمر. افتحه لتعرف شو ملاحظ، شو ممكن يعني، وشو الفحص التالي.`:`Tank Intelligence continuously reads ${tank.name}'s state, history and trends. Open it to see what it notices, what it may mean, and what to check next.`;
 ...
 return <div className={`aqua-ai-shell ${open?"open":""} state-${state}`} dir={lang==="ar"?"rtl":"ltr"}>
  {greeting&&!open&&<button type="button" className="aqua-ai-greeting" onClick={()=>{setGreeting(false);setOpen(true)}}><b>{lang==="ar"?"Tank Intelligence":"Tank Intelligence"}</b><span>{greetingText}</span></button>}
  {open&&<section className="aqua-ai-panel aqua-ai-panel-v2">
   <div className="aqua-ai-head"><div className="aqua-ai-head-brand"><FishMascot state={state}/><div><b>✦ Tank Intelligence</b><small>{lang==="ar"?"ذكاء Aqua Nexus الداخلي • خاص بالحوض الحالي":"Aqua Nexus internal intelligence • current tank only"}</small></div></div><button className="icon-btn" onClick={()=>setOpen(false)}>×</button></div>
   ...
   <div className="aqua-ai-answer aqua-ai-structured-answer">
    ...
    <div className="aqua-ai-local-note">{lang==="ar"?"Tank Intelligence ليس شات عام. هو جزء داخلي من Aqua Nexus ويحلل فقط بيانات الحوض الحالي واتجاهاته وسجلّه.":"Tank Intelligence is not a general chatbot. It is an internal Aqua Nexus system that analyzes only the current tank, its trends and history."}</div>
  ...
  <button className="aqua-ai-fish-button" onClick={()=>{setGreeting(false);setOpen(v=>!v)}} aria-label="Tank Intelligence"><FishMascot state={state}/><span className="aqua-ai-fish-label">Tank Intelligence</span>{state!=="normal"&&<i className="aqua-ai-alert-dot"/>}</button>
 </div>;
}
