...
export function AquaAIAssistant({tank,page,onNavigate}:{tank:Tank;page:AppPage;onNavigate?:(page:AppPage)=>void}){
 ...
 const [open,setOpen]=useState(false),[q,setQ]=useState(""),[history,setHistory]=useState<Exchange[]>([]),[greeting,setGreeting]=useState(false),[planNote,setPlanNote]=useState("");
 ...
 const activeQuestion=history[history.length-1]?.question||(lang==="ar"?"تحليل الحوض":"Tank analysis");
 const plans:AquaActionPlan[]=((tank as any).aiActionPlans??[]);
 const currentPlan=plans.find(x=>x.status==="active");

 useEffect(()=>{setHistory([]);setQ("");setPlanNote("")},[tank.id]);
 ...
 function run(question:string){const clean=question.trim()||(lang==="ar"?"حلل حوضي":"Analyze my tank");const answer=aquaAIAnswer(clean,tank,page);setHistory(h=>[...h.slice(-4),{id:`ai-${Date.now()}`,question:clean,answer}]);setQ("");}
 ...
 return <div className={`aqua-ai-shell ${open?"open":""} state-${state}`} dir={lang==="ar"?"rtl":"ltr"}>
  ...
  {open&&<section className="aqua-ai-panel aqua-ai-panel-v2">
   <div className="aqua-ai-head"><div className="aqua-ai-head-brand"><FishMascot state={state}/><div><b>✦ Aqua AI</b><small>{lang==="ar"?"عقل الحوض المحلي • Tank-aware":"Local tank intelligence • Tank-aware"}</small></div></div><button className="icon-btn" onClick={()=>setOpen(false)}>×</button></div>
   ...
   <div className="aqua-ai-answer aqua-ai-structured-answer">
    ...
    <div className="aqua-ai-local-note">{lang==="ar"?"التحليل من محرك Aqua Nexus المحلي. تحليل الصور الحالي فرز سياقي محلي وليس تشخيص Vision Model خارجي بعد.":"Analysis comes from Aqua Nexus' local reasoning engine. Current image analysis is local contextual triage, not yet an external vision-model diagnosis."}</div>
