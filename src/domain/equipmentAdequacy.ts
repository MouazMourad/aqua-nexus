...
export function equipmentAdequacy(tank:Tank):EquipmentAdequacyResult{
  ...
  if(tank.type==="freshwater"&&!has(tank,"canisterFilter")&&!has(tank,"spongeFilter")&&!has(tank,"filterSock")&&!has(tank,"rollerFilter")){
    issue({id:"missing-freshwater-filtration",level:"danger",ar:"لا توجد وسيلة فلترة فعّالة مسجلة للحوض العذب.",en:"No active filtration device is registered for this freshwater tank.",recommendationAr:"سجل Canister Filter أو Sponge Filter أو وسيلة الفلترة الفعلية قبل اعتبار التجهيزات الأساسية مكتملة.",recommendationEn:"Register the actual Canister Filter, Sponge Filter, or other active filtration before considering essentials complete."},35);
  }

  if(profile==="freshwater-planted"){
    const co2=active(tank,"co2");
    if(!co2.length)suggest({id:"co2-mode",kind:"co2",level:"info",ar:"ما في منظومة CO₂ مسجلة. هذا طبيعي إذا الحوض Low-tech؛ لا يعتبر CO₂ إلزامياً لكل حوض مزروع.",en:"No CO₂ system is registered. That is normal for a low-tech planted tank; injected CO₂ is not mandatory for every planted aquarium.",recommendationAr:"إذا الحوض High-tech ويعتمد حقن CO₂، أضف منظومة CO₂ حتى تدخل السلامة والصيانة والمخزون ضمن العقل.",recommendationEn:"If this is a high-tech tank using injected CO₂, register the CO₂ system so safety, maintenance and inventory become part of tank intelligence."});
    for(const e of co2){if(e.co2CylinderRemainingPercent!==undefined&&e.co2CylinderRemainingPercent<=15)suggest({id:`co2-low-${e.id}`,kind:"co2",level:e.co2CylinderRemainingPercent<=5?"warn":"info",ar:`${e.name}: الأسطوانة المتبقية تقريباً ${e.co2CylinderRemainingPercent}%.`,en:`${e.name}: cylinder remaining is about ${e.co2CylinderRemainingPercent}%.`,recommendationAr:"جهّز التعبئة/البديل قبل نفاد الغاز وتغير ثبات الحقن.",recommendationEn:"Prepare a refill/spare before gas runs out and injection stability changes."});if(e.co2DropChecker==="yellow")issue({id:`co2-yellow-${e.id}`,kind:"co2",level:"danger",ar:`${e.name}: Drop checker مسجل أصفر؛ هذا قد يشير إلى CO₂ مرتفع ويحتاج تحقق فوري.`,en:`${e.name}: drop checker is recorded yellow; this can indicate excessive CO₂ and needs immediate verification.`,recommendationAr:"خفف/أوقف الحقن مؤقتاً حسب الحالة، زِد التهوية، وراقب الأسماك والتنفس قبل أي تعديل إضافي.",recommendationEn:"Reduce/pause injection as appropriate, increase aeration, and observe fish respiration before further adjustment."});}
  }
