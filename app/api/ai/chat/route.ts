...
import { declaredBodyTooLarge,enforceRateLimit,publicApiError,sanitizeAIQuestion } from "@/server/requestSafety";

export const runtime="nodejs";

export async function POST(request:NextRequest){
  try{
    if(declaredBodyTooLarge(request,4*1024*1024))return NextResponse.json({ok:false,error:"AI request exceeds the server size limit."},{status:413});
    const workspace=workspaceKey(request);
    const rate=enforceRateLimit(`ai-chat:${workspace}`,30);
